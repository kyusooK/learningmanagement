<template>
    <div>
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>LectureId</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <LectureId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import LectureId from '../LectureId.vue';

    export default {
        name: 'LectureIdManager',
        components: {
            LectureId
        },
        props: {
