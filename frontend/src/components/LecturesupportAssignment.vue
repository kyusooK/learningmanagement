<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            과제 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            과제
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <String label="피드백" v-model="value.feedback" :editMode="editMode" :inputUI="''"/>
        </v-card-text>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/assignments'), this.value)
                        } else {
