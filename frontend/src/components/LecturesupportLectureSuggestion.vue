<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            강의추천 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            강의추천
        </v-card-title >        
...
<script>
    ...
    export default {
        name: 'LecturesupportLectureSuggestion',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/lectureSuggestions'), this.value)
                        } else {
