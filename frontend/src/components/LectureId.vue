<template>
    <v-card outlined @click="openDialog">
        <v-card-title>
            Lecture : {{ referenceValue ? referenceValue._links.self.href.split('/').pop() : '' }}
        </v-card-title>

        <v-dialog v-model="pickerDialog" width="500">
            <v-card>
                <v-card-title>Lecture</v-card-title>
                <v-card-text>
                    <LecturePicker v-model="value" @selected="pick"/>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        methods: {
            ...
            openDialog() {
                ...
                if(this.editMode) {
                    ...
                    this.$router.push(path + this.value.lectureId);
                }
            },
            async pick(val){
                this.newValue = val;
                var path = '/lectures';
                var temp = await axios.get(axios.fixUrl(path + '/' + val.lectureId));
                if(temp.data) {
                    this.referenceValue = temp.data;
                }
