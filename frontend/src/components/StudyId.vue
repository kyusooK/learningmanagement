<template>
    <v-card outlined @click="openDialog">
        <v-card-title>
            Study : {{ referenceValue ? referenceValue._links.self.href.split('/').pop() : '' }}
        </v-card-title>

        <v-dialog v-model="pickerDialog" width="500">
            <v-card>
                <v-card-title>Study</v-card-title>
                <v-card-text>
                    <StudyPicker v-model="value" @selected="pick"/>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
        ...
        methods: {
            ...
            openDialog() {
                ...
                if(this.editMode) {
                    ...
                    this.$router.push(path + this.value.studyId);
                }
            },
            async pick(val){
                this.newValue = val;
                var path = '/studies';
                var temp = await axios.get(axios.fixUrl(path + '/' + val.studyId));
                if(temp.data) {
                    this.referenceValue = temp.data;
                }
